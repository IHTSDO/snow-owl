...
public class SnomedBrowserService implements ISnomedBrowserService {
	...
	@Override
	public Set<ISnomedBrowserConcept> getConceptDetailsInBulk(final String branchPath, final Set<String> conceptIds, final List<ExtendedLocale> locales) {
		...
		return getAxiomService().expandAxioms(browserConcepts, branchPath, locales).stream().collect(toSet());
	}
	...
	@Override
	public List<ISnomedBrowserParentConcept> getConceptParents(final String branchPath, final String conceptId, final List<ExtendedLocale> locales, boolean stated, SnomedBrowserDescriptionType preferredDescriptionType) {
		...
		return new FsnJoinerOperation<ISnomedBrowserParentConcept>(conceptId, locales, descriptionService, preferredDescriptionType) {
			...
			protected Iterable<SnomedConcept> getConceptEntries(String conceptId) {
				SnomedConcept concept = SnomedRequests.prepareGetConcept(conceptId)
						.setExpand(stated ? "statedAncestors(direct:true)" : "ancestors(direct:true)")
						.setLocales(locales)
						.build(SnomedDatastoreActivator.REPOSITORY_UUID, branchPath)
						.execute(getBus())
						.getSync();
	...
	@Override
	public List<ISnomedBrowserChildConcept> getConceptChildren(final String branchPath, final String conceptId, final List<ExtendedLocale> locales, final boolean stated, 
	...
	@Override
	public List<ISnomedBrowserDescriptionResult> getDescriptions(final String branchPath, final String query, final List<ExtendedLocale> locales, final SnomedBrowserDescriptionType preferredDescriptionType, final int offset, final int limit) {
	...
	private void update(String branchPath, ISnomedBrowserConcept newVersionConcept, ISnomedBrowserConcept existingVersionConcept, String userId, List<ExtendedLocale> locales, 
		...
		if (!existingVersionConcept.getAdditionalAxioms().isEmpty() || !newVersionConcept.getAdditionalAxioms().isEmpty()) {
			...
			List<SnomedRefSetMemberCreateRequest> newAdditionalAxiomRequests = inputFactory.createComponentInputs(branchPath, newAdditionalAxioms, SnomedRefSetMemberCreateRequest.class);
		...
		if (!existingVersionConcept.getGciAxioms().isEmpty() || !newVersionConcept.getGciAxioms().isEmpty()) {
			...
			// gci axiom create
			List<SnomedRefSetMemberCreateRequest> newGciAxiomRequests = inputFactory.createComponentInputs(branchPath, newGciAxioms, SnomedRefSetMemberCreateRequest.class);
